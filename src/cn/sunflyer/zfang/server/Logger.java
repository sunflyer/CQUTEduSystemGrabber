...
public class Logger {
	...
	public static void initLogger(String FilePath){
		gLogPath=FilePath;
		gLogFile=new File(gLogPath,Logger.gLogFileName);
		try{
			if(!gLogFile.exists()){
				gLogFile.createNewFile();
			}
			mLogger = new FileWriter(Logger.gLogFile,true);
			gIsInit=true;
		}
		catch(Exception ex){
			ex.printStackTrace();
			gIsInit=false;
			Logger.gInitErrorTime++;
		}
	}
	...
	public static void log(String pCon){
		if(isInit()){
			try {
				mLogger.write(new Date()+"："+pCon+Logger.nLine);
				mLogger.flush();
			} catch (IOException e) {
				e.printStackTrace();
			}
		}else{
			if(Logger.gInitErrorTime<Logger.gInitErrorTimeLimit){
	...
	public static void logE(Exception e){
		if(isInit()){
			try {
				mLogger.write((new Date()).toString()+"：################发现异常################"+Logger.nLine+e.getMessage()+Logger.nLine+Logger.getStackTrace(e)+Logger.nLine);
				mLogger.flush();
	...
	public static String getStackTrace(Exception e){
		...
		for(int i=0;i<pSte.length;i++)
			pSb.append(pSte[i].toString()+Logger.nLine);
		return pSb.toString();
	}
