...
public class Grabber {
	...
	public static void main(String[] args){
		...
		**/
		...
		ArrayList<GradeInfo> p = getGrade(pUi);
...
if(pGrade == null) return;
	...
	public static void saveGrade(UserInfo i){
		saveGrade(getGrade(i),"stugrade-" + i.no + "-" + i.name);
	}
	
	public static void saveGrade(ArrayList<GradeInfo> p,UserInfo i){
		saveGrade(p,"stugrade-" + i.no + "-" + i.name);
	}
	
	/**
	 * Print As Html
	 * */
	public static void saveGrade(ArrayList<GradeInfo> pGrade,String name){
		...
		if(pGrade != null){
			StringBuffer pSb = new StringBuffer("<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><style>td{min-width:100px;}</style><title>" + name + "</title></head><body><table>");
			pSb.append("<tr><td>学年</td><td>学期</td><td>课程代码</td><td>课程名称</td><td>课程学分</td><td>绩点</td><td>成绩</td><td>重修标记</td></tr>");
			for(GradeInfo x:pGrade){
				pSb.append("<tr><td>" + x.stuYear + "</td><td>" + x.stuPeriod + "</td><td>" + x.classCode + "</td><td>" + x.className + "</td><td>" + x.credit + "</td><td>" + x.gradeNorm + "</td><td>" + x.gradeMid + "</td><td>" + x.isRestudy + "</td></tr>");
			}
			pSb.append("</table><p>平均绩点：" + getGradePoint(pGrade) + "</p><p>Automatic Generated By CQUTEduSystemGrabber @ " + ((new Date()).toString()) + " <a href=\"https://github.com/sunflyer/CQUTEduSystemGrabber\">GitHub</a></p></body></html>");
			try {
		...
		if(i != null){
			...
			for(GradeInfo x:i){
				if(pHm.containsKey(x.classCode)){
					try{
						if(Double.valueOf(x.gradeMid) > Double.valueOf(pHm.get(x.classCode).gradeMid)){
							pHm.put(x.classCode, x);
						}
					}catch(Exception e){
						if(getGradeLevel(x.gradeMid) > getGradeLevel(pHm.get(x.classCode).gradeMid)){
							pHm.put(x.classCode, x);
						}
					}
				}else{
			...
			for(GradeInfo x:pHm.values()){
				...
				point += Double.valueOf(x.gradeNorm) * Double.valueOf(x.credit);
			}
